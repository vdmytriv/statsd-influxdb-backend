...
function InfluxdbBackend(startupTime, config, events) {
  ...
  self.defaultHost = '127.0.0.1';//change to metrics qa
  ...
  //upgrade to 0.11 which will remove json protocol, tested & works up for 0.14
...
InfluxdbBackend.prototype.httpPOST_v09 = function (points) {
  ...
  req.write(payload);
  req.end();
} //end of InfluxdbBackend.prototype.httpPOST_v09
...
InfluxdbBackend.prototype.httpPOST_v11 = function (points) {
  ...
  self.logDebug(function () {
    return 'Sending ' + points.length + ' different points via ' + protocolName + ' version: ' + self.version;
  });
  ...
  //vita zhao add the user and password at 5/24/2016
    path: '/write?db=' + self.database+'&rp='+self.retentionPolicy+'&u='+self.user+'&p='+self.pass,//TODO:/write?db=dbname
    method: 'POST',
    headers:{
//        'content-type': 'binary'
  ...
  req.on('response', function (res) {
    ...
    if (status !== 200) {
      self.log(protocolName + ' Warning: ' + status);
    }
  });
...
//end of InfluxdbBackend.prototype.httpPOST_v11
